...
class JDBCSink(url: String, user: String, pwd: String)
  ...
  def process(value: org.apache.spark.sql.Row): Unit = {
    statement.executeUpdate(
      "INSERT INTO emp_o1(id, last_name) " +
...
// E.g.: $>  cp t4.csv ../input/
  ...
  private def readJDBC(spark: SparkSession): Unit = {
    ...
    val driver = "org.postgresql.Driver"
    val url = "jdbc:postgresql://localhost:5432/report?user=...&password=..."
    Class.forName(driver)
// var connection: Connection = _
    ...
    // read.jdbc with argument connectionProperties
    val emp_df = spark.read.jdbc(url, "games", connectionProperties)
    emp_df.show
    ...
    val df2 = spark.read
      ...
      .option("dbtable", "(select game_id,game_name from games) e ")
      .option("user", "...")
      .option("password", "...")
      .load()
    df2.printSchema
...
// emp_df.show
    ...
    val df2 = spark.read
      .format("jdbc")
      .option("url",
              "jdbc:postgresql://localhost:5432/scala_db?user=scala_user")
...
//     .start()
  ...
  private def monitorStream(spark: SparkSession): Unit = {
